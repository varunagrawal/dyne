...
dyne::CombinedImuBase GetEstimator() {
  gtsam::imuBias::ConstantBias bias(gtsam::Vector3::Zero(),
                                    gtsam::Vector3::Zero());
  dyne::RobotImu base_imu(200, 5.886e-4, 1.745e-4, gtsam::Pose3(), bias);

  dyne::CombinedImuBase estimator(base_imu, 50);
  return estimator;
}

/* ************************************************************************/
TEST(CombinedImuBase, Constructor) {
  dyne::CombinedImuBase estimator = GetEstimator();
}

/* ************************************************************************/
TEST(CombinedImuBase, Preintegrate) {
